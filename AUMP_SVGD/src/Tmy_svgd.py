...
class etmySVGD:
    ...
    def __init__(self, kernel, device,distribution,k=2):
        self.k = kernel
        self.d = device
    ...
    def divide(self, theta):
        ...
        max_list = torch.topk(list, k=self.b, dim = 0, largest=True, sorted=False)
        support = max_list.indices

        min_list = torch.topk(list, k=theta.shape[1]-self.b, dim = 0, largest=False, sorted=False)
    ...
    def fit(self, x0: torch.Tensor, epochs: torch.int64, verbose: bool = True,
        ...
        for ep in iterator:
            for j, (X_batch, y_batch) in enumerate(train_loader):
                theta = x0.clone()
                theta = self.step(x0,theta,  lr = 0.0001, X_batch=X_batch,  y_batch=y_batch)
