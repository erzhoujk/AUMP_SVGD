...
from src.Tmy_svgd import etmySVGDLR
from src.myysvgd import myySVGDLR
...
parser.add_argument("--epochs", type=int, default=40, help="no. of epochs")
...
if __name__ == "__main__":
    ...
    if args.data == "covertype":
        data = scipy.io.loadmat("/home/zhoujk/SVGD/SVGD_code/GSVGD-main/data/covertype.mat")
    ...
    start = time.time()
    svgd.fit(x0=x, epochs=20, save_every=save_every, train_loader=train_loader,
    ...
    kernel = Kernel(method="med_heuristic")
    mysvgd = etmySVGDLR(kernel, device, distribution,10)
    mysvgd.fit(x, epochs=100, save_every=save_every, train_loader=train_loader,
        test_data=(X_test, y_test), valid_data=(X_valid, y_valid))

    fitted_method = mysvgd
    method_name = "mysvgd"
