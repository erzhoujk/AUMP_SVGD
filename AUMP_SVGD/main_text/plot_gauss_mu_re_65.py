...
if __name__ == "__main__":
    ...
    correlation=0.01
        ...
        def run_mysvgd(k = 2, score=score,lr=0.01):
            x0 = x_init.clone().to(device)
            vector1  = torch.zeros_like(x0).to(device)
            for i in trange(100):
    
                theta, vector = mySVGD(device).update(x0, score=score,  k = k, n_iter = 50,  debug = False, lr= lr, vector=vector1)
    ...
    #x0 = np.random.multivariate_normal(mean, var_theta,num)
        ...
        theta = run_mysvgd(k = 2, score=score,lr=0.01)
        ...
        theta = run_mysvgd(k = 3, score=score,lr=0.01)
        ...
        theta = run_mysvgd(k = 5, score=score,lr=0.01)
    ...
    torch.save(list_norm, "norm_gauss_mu_vs_110_1.pt")
    torch.save(list_gr_var, "gr_gauss_mu_vs_110_1.pt")
    torch.save(list_tr, "tr_gauss_mu_vs_110_1.pt")
    torch.save(list_eig, "eig_gauss_mu_vs_110_1.pt")
    torch.save(list_energy, "energy_gauss_mu_vs_110_1.pt")